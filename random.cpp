...
void extrapolate(double E[],double E2[],double n,double m,double res[])
{
	for(int i = 0; i < n; i++)
	{
		double k = i+1;
		res[i] = (pow(n,5)*sin((k*pi)/n)*E[i] - pow(m,5)*sin((k*pi)/m)*E2[i])/(pow(n,5)*sin((k*pi)/n) - pow(m,5)*sin((k*pi)/m));
	}
}
double p_f(double x)
{
